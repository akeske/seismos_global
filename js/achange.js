...
function load() {
	var latlng = new google.maps.LatLng(21, 145);
	var settings = {
		zoom: 2,
		center: latlng,
		scrollwheel: false,
	...
	downloadJson("planet-predictions.php", function (data) {

		let planetPredictions = JSON.parse(data);

		for(let [planetPredictionIndex, planetPrediction] of planetPredictions.entries()) {
			let markersPlanetPrediction = {};
			for(let [predictionIndex, prediction] of planetPrediction.predictions.entries()) {
				for(let [pointIndex, point] of prediction.predictions.entries()) {
					let lat = point.latitude;
					let lng = point.longitude;
					let latlng = new google.maps.LatLng(lat, lng);
					...
					markersPlanetPrediction[prediction.name].push(marker) ;
				}
			}
			markersPlanetPredictions[planetPrediction.name] = markersPlanetPrediction;

			addPlanetPredictionTable(planetPrediction.name);
		}
	});
	...
	function addPlanetPredictionTable( name ){
		let table = document.getElementById('planet-prediction-table');
		...
		row.insertCell(15).innerHTML = "<a href=\"#!\" id=\"planetPredictionsDisplay"+ name +"Bl\" onclick=\"planetPredictionDisplay('" + name +"','Bl')\"><img src='images/" + (typeof markersPlanetPredictions[name]['Bl'] === 'undefined' ? 'disabled':'show') + ".png'/></a>";
	}
...
//		var temp;
//		temp = latchess[0];
	//	latchess[0] = latchess[1];
//		latchess[1] = temp;
//		temp = lngchess[0];
	//	lngchess[0] = lngchess[1];
	//	lngchess[1] = temp;
	//	alert( lngchess[0]  );
	//	alert( lngchess[1]  );
		...
		//			latlng2 = new google.maps.LatLng(latchess[j], lngchess[size]);
				...
				if( (lngchess[0]>=0 && lngchess[1]>=0 ) || (lngchess[0]<=0 && lngchess[1]<=0 )  ){
					var finlatlng2 = [
						new google.maps.LatLng(latchess[j], lngchess[0]),
						new google.maps.LatLng(latchess[j], lngchess[1])
					];
				}else{
					var finlatlng2 = [
						new google.maps.LatLng(latchess[j], lngchess[0]),
						new google.maps.LatLng(latchess[j], 180),
						new google.maps.LatLng(latchess[j], lngchess[1])
					];
				}
				line1.push(latlng1);
				line2.push(finlatlng2);
	//			alert( line2 );
			}
	...
	downloadUrl("phpsqlajax_xmlD1.php", function(data) {
		...
		for ( j = 0; j < size * size; j++) {
		    ...
		    for ( i = 0; i < markersXML.length; i++) {
		        ...
		        var marker = createMarker(id, latlng, megethos, vathos, type, typeSize, date, l, lat, lng, qntMarkers);
		        markers.push( marker );
...
function setAllMap(map) {
	...
	for (var i = 0; i < markers.length; i++) {
		var max = markers.length-1;
	  	if( i==max ){
			tempType = parseInt( markers[i].getTitle() );
			tempType += 6;
			markers[i].setIcon( customIcons[tempType] );
...
function markersShowHide() {
	...
	//	counter = markersPeriod1.length;
		counter = markers.length;
		document.getElementById('coordslatlast').value = markers[counter-1].getPosition().lat().toFixed(2);
		document.getElementById('coordslnglast').value = markers[counter-1].getPosition().lng().toFixed(2);
		setAllMap(map);
	}
}
function nextMarker() {
	if( counter<markers.length ){
		var tempType = 0;
		if( counter==0 ){
			tempType = parseInt( markers[counter].getTitle() );
			tempType = tempType + 6;
			markers[counter].setIcon( customIcons[tempType] );
			markers[counter].setMap(map);
			document.getElementById('coordslatlast').value = markers[counter].getPosition().lat().toFixed(2);
			document.getElementById('coordslnglast').value = markers[counter].getPosition().lng().toFixed(2);
			counter++;
		}else{
			counter--;
			tempType = parseInt( markers[counter].getTitle() );
...
function Label(opt_options) {
	...
	span.style.cssText = 'position: relative; left: -45%; top: -20px; ' +
		...
		'padding: 1px; background-color: white; z-index: 100; font-size:14px;';
...
function setAllMapPlanetPred(map, filename, planet) {
	for(let pp of markersPlanetPredictions[filename][planet]) {
		pp.setMap(map);
	}
}
...
function geodesic() {
	if( document.getElementById("geodesicHidden").value == 0 ){
		document.getElementById("geodesicHidden").value = 1;
	}else{
		document.getElementById("geodesicHidden").value = 0;
	}
}